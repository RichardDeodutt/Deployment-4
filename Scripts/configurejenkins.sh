...
main(){
    ...
    #Format IP
    mv JENKINS_IP JENKINS_IP_TEMP && echo "http://$JENKINS_IP_TEMP/" > JENKINS_IP && rm JENKINS_IP_TEMP && logokay "Successfully formatted IP for ${Name}" || { logerror "Failure formatting IP for ${Name}" && exiterror ; }
