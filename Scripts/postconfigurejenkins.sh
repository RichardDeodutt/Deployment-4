...
main(){
    ...
    cat $ConfigSecretJenkinsFileName | java -jar $JCJ -s "http://localhost:8080" -http -auth $JENKINS_USERNAME:$JENKINS_PASSWORD create-credentials-by-xml system::system::jenkins _ > JenkinsExecution && logokay "Successfully executed send secret config for ${Name} AWS_ACCESS_KEY_ID" || { test $? -eq 1 && logwarning "Secret config for ${Name} AWS_ACCESS_KEY_ID already exists nothing changed" || { logerror "Failure executing send secret config for ${Name} AWS_ACCESS_KEY_ID" && cat JenkinsExecution && rm JenkinsExecution && exiterror ; } ; }
    ...
    cat $ConfigSecretJenkinsFileName | java -jar $JCJ -s "http://localhost:8080" -http -auth $JENKINS_USERNAME:$JENKINS_PASSWORD create-credentials-by-xml system::system::jenkins _ > JenkinsExecution && logokay "Successfully executed send secret config for ${Name} AWS_SECRET_ACCESS_KEY" || { test $? -eq 1 && logwarning "Secret config for ${Name} AWS_SECRET_ACCESS_KEY already exists nothing changed" || { logerror "Failure executing send secret config for ${Name} AWS_SECRET_ACCESS_KEY" && cat JenkinsExecution && rm JenkinsExecution && exiterror ; } ; }
