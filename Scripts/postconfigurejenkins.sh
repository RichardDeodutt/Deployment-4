...
ConfigSecretJenkins="https://raw.githubusercontent.com/RichardDeodutt/Deployment-4/main/Configs/credential-secret-jenkins-default"
...
main(){
    ...
    java -jar $JCJ -s "http://localhost:8080" -http -auth $JENKINS_USERNAME:$JENKINS_PASSWORD create-credentials-by-xml system::system::jenkins _ < $(cat $ConfigSecretJenkinsFileName) && logokay "Successfully executed send secret config for ${Name} AWS_ACCESS_KEY_ID" || { logerror "Failure executing send secret config for ${Name} AWS_ACCESS_KEY_ID" && exiterror ; }
    ...
    java -jar $JCJ -s "http://localhost:8080" -http -auth $JENKINS_USERNAME:$JENKINS_PASSWORD create-credentials-by-xml system::system::jenkins _ < $(cat $ConfigSecretJenkinsFileName) && logokay "Successfully executed send secret config for ${Name} AWS_SECRET_ACCESS_KEY" || { logerror "Failure executing send secret config for ${Name} AWS_SECRET_ACCESS_KEY" && exiterror ; }
